...
int main(int argc, char **argv)
{
    if(argc != 4)
    {
        cerr << endl << "Usage: ./mono_tum path_to_vocabulary path_to_settings_and_sequence path_to_sequence path_to_workspace" << endl;
        return 1;
    }
    ...
    ORB_SLAM3::System SLAM(argv[1],string(argv[3])+"/ICL.yaml",ORB_SLAM3::System::MONOCULAR,true);
    ...
    for(int ni=0; ni<nImages; ni++)//每张图片
    {
        ...
        im = cv::imread(string(argv[2])+"/rgb/"+to_string(ni)+".png",cv::IMREAD_UNCHANGED);
        //im = cv::imread("/home/changfei_fu/dataset/ICLNUIM_RGBD_dataset/living_room_traj0_frei_png/rgb/"+to_string(ni)+".png",cv::IMREAD_UNCHANGED);
        double tframe = vTimestamps[ni];

        if(im.empty())//判断图像加载成功
        {
            im = cv::imread(string(argv[2])+"/rgb/"+to_string(ni+1)+".png",cv::IMREAD_UNCHANGED);
            if(im.empty()){
