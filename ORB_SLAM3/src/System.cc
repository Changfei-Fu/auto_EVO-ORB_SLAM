...
void System::SaveKeyFrameTrajectoryTUM(const string &filename)
{
    ...
    /*这一段从System::SaveKeyFrameTrajectoryEuRoC()中复制过来，为了选取最长的一段*/
...
void System::SaveKeyFrameTrajectoryEuRoC(const string &filename)
{
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO)
        {
            ...
            f << setprecision(0) << 1e9*pKF->mTimeStamp  << " " <<  setprecision(9) << twb.at<float>(0) << " " << twb.at<float>(1) << " " << twb.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
        ...
        else
        {
            ...
            string whole_string("");
            string head6(""),rail13("");
            ss<<std::fixed<<setprecision(9)<<pKF->mTimeStamp;
            ss>>whole_string;

            head6=whole_string.substr(0,6);
            rail13=whole_string.substr(6);
            ...
            f << head6 << std::fixed << setprecision(0) << time << " " <<  setprecision(6) << t.at<float>(0) << " " << t.at<float>(1) << " " << t.at<float>(2) << " " << q[0] << " " << q[1] << " " << q[2] << " " << q[3] << endl;
        }
    }
    f.close();
}
