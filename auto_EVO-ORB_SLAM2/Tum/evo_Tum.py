...
def kft_Gen(data_Path,Index):
    ...
    if not os.path.exists(results_Path+"/KeyFrameTrajectory"+str(Index)+".txt"):
        os.system("cp -r "+data_Path+"/rgb ./") 

        os.system("cp "+results_Path+"/rgb.txt ./")

        orbslam_Path="../../ORB_SLAM2"
        orbslam=orbslam_Path + "/Examples/Monocular/mono_tum_evo"
        orbslam_Voc=orbslam_Path+"/Vocabulary/ORBvoc.txt"
        if data_Name.find("freiburg1") > 0:
            Intrinsics="./TUM1.yaml"
        elif data_Name.find("freiburg2") > 0:
            Intrinsics="./TUM2.yaml"
        elif data_Name.find("freiburg3") > 0:
            Intrinsics="./TUM3.yaml"

        cmd = orbslam+" "+orbslam_Voc+" "+Intrinsics+" ./"
        time1=time.time()
        os.system(cmd)
        time2=time.time()
        time3=time2-time1

        if not os.path.exists("./KeyFrameTrajectory.txt"):
            time3 = kft_Gen(data_Path,Index)
        else :
            cmd="mv KeyFrameTrajectory.txt "+results_Path+"/KeyFrameTrajectory"+str(Index)+".txt"
            os.system(cmd)
            os.system("rm -rf ./rgb ./rgb.txt")
    return time3
...
def evo_data(data_Path,times):#输入某个数据集地址
    ...
    data_Name = os.path.basename(data_Path)
    results_Path="./Results/"+data_Name
    if not os.path.exists("./tracking_Rate") :
